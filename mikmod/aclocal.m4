...
dnl LIBMIKMOD_LDADD
dnl
AC_DEFUN(AM_PATH_LIBMIKMOD,[dnl 
dnl
...
AC_ARG_WITH(libmikmod-prefix,[  --with-libmikmod-prefix=PFX   Prefix where libmikmod is installed (optional)],
			libmikmod_config_prefix="$withval",
			libmikmod_config_prefix="")
AC_ARG_WITH(libmikmod-exec-prefix,[  --with-libmikmod-exec-prefix=PFX Exec prefix where libmikmod is installed (optional)],
			libmikmod_config_exec_prefix="$withval",
			libmikmod_config_exec_prefix="")
AC_ARG_ENABLE(libmikmodtest,[  --disable-libmikmodtest       Do not try to compile and run a test libmikmod program],,enable_libmikmodtest=yes)
...
if test "$LIBMIKMOD_CONFIG" = "no" ; then
	...
	libmikmod_config_major_version=`$LIBMIKMOD_CONFIG $libmikmod_config_args --version | sed 's/\([[0-9]]*\).\([[0-9]]*\).\([[0-9]]*\).*/\1/'`
	libmikmod_config_minor_version=`$LIBMIKMOD_CONFIG $libmikmod_config_args --version | sed 's/\([[0-9]]*\).\([[0-9]]*\).\([[0-9]]*\).*/\2/'`
	libmikmod_config_micro_version=`$LIBMIKMOD_CONFIG $libmikmod_config_args --version | sed 's/\([[0-9]]*\).\([[0-9]]*\).\([[0-9]]*\).*/\3/'`
...
if test "x$no_libmikmod" = x ; then
	LIBMIKMOD_VERSION=`$LIBMIKMOD_CONFIG $libmikmod_config_args --version`
	AC_MSG_RESULT([yes, $LIBMIKMOD_VERSION])
	ifelse([$2], , :, [$2])     
else
	AC_MSG_RESULT(no)
	if test "$LIBMIKMOD_CONFIG" = "no" ; then
