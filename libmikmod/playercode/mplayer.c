		...
		if (t->pts > 1)	{
			...
			for (i = 0; i < t->pts - 1; i++) {
				if (tick < p[idx].pos) {
					...
					t->lastval = InterpolateEnv(tick, &t->env[idx], &t->env[idx + 1]);
					t->interpolate = 1;
...
static inline void ProcessEnvelopeXm(MP_VOICE *aout, ENVPR *t)
{
	...
	if (t->interpolate)
		t->lastval = InterpolateEnv(t->tick, &t->env[idx - 1], &t->env[idx]);
}
...
static inline void ProcessEnvelopeIt(MP_VOICE *aout, ENVPR *t)
{
	...
	if ((pointPos > newPos) && (tick > newPos))
		t->lastval += ((tick - newPos) * (t->env[idx].val - t->lastval)) / (pointPos - newPos);
}
